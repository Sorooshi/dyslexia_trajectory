...
def AUC(y_test_hot, y_pred_proba):
    return roc_auc_score(y_test_hot, y_pred_proba, average="weighted")

def ACC(y_test_labels, y_pred_labels):
    return accuracy_score(y_test_labels, y_pred_labels)

def PR(y_test_labels, y_pred_labels):
    return precision_score(y_test_labels, y_pred_labels, average="weighted")

def RECC(y_test_labels, y_pred_labels):
    return recall_score(y_test_labels, y_pred_labels, average="weighted")

def F1(y_test_labels, y_pred_labels):
    return f1_score(y_test_labels, y_pred_labels, average="weighted")


def metrics_per_fold(model, test_dataset, metrics_dict):
    fl = False
