...
def splitting_window_fused(features, targets, n_steps):
    x_seq, y_seq = [], []
...
def window_dataset_creation_fused(n_steps, path, dataset_name):
    ...
    x_train_windowed_final, y_train_windowed_final = splitting_window_fused(x_train.values, y_train.values, n_steps)
    ...
    for ids in data["SubjectID"].unique()[1:]:
        ...
        y_train = df_id.loc[:, ["Group"]]
        x_train = df_id.loc[:, ["FIX_X", "FIX_Y", "FIX_DURATION", "Age"]]
        x_train_windowed, y_train_windowed = splitting_window_fused(x_train.values, y_train.values, n_steps)
